...
Before making this repo public: replaced engagement-specific example strings throughout the skill
docs and script usage blocks with neutral generics (`/tmp/target`, `ParseRequest`, `--bv-match
target`, `./symdb.sqlite`). Example command lines that name a real target binary leak the *direction*
of live research, which is a disclosure problem independent of any secret.

Added `test_repo_hygiene()` — the existing "no vendor strings" check only walked `skills/sbom-kb`,
so the other eight skills were unenforced (and sbom-kb itself had 5 failing assertions). Now two
tiers: engagement-identifying tokens (target binary/host/build/workspace names) are banned repo-wide
with reviewed exemptions, while generic vendor words stay enforced in `sbom-kb` only — the shipped
`esxi-vmkernel` / `vmx-userworld` / `vmware` profiles and the METHODOLOGY provenance are legitimate
and unchanged.

Also: manifest version was stale at 0.12.0 against a 0.13.0 changelog; both now 0.13.1.
...
- All engagement data stripped; ships with an example seed and empty reference dir.
