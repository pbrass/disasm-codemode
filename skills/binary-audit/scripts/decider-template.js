export const meta = {
  ...
  description: 'Iterative-deepening decider audit: resolve each uncertain corruption/disclosure/race/lifetime bug by auditing its named upstream decider',
...
const PROMPT=(t)=>`PHASE-2 DECIDER audit (authorized ESXi 8.0.3i vmkernel pentest). A prior trace returned UNCERTAIN on a suspected bug and named THIS function as the decider. Determine whether it establishes the relevant safety precondition (established-safe), fails to (violable-bug), or itself defers upstream (uncertain → name the next).
...
CONSUMER (the exact precondition/primitive that must hold): ${t.consumer.name}: HLIL ${t.consumer.hlil} | ASM ${t.consumer.asm}
...
- established-safe: this function (or one it provably calls) clamps/bounds the value, copies guest state once to a stable local, holds the needed ref/lock, initializes the exposed data, checks the type, or derives it from a fixed constant. The bug cannot fire.
- violable-bug: you can point to the concrete unsafe primitive AND a guest/target input or state transition that reaches it unguarded — a clean confirmed bug. Set guest_reachable_path.
