...
for _col in ("bug_class TEXT","leak_back TEXT","disclosure_source TEXT","reachability TEXT","guarded_by TEXT","impact TEXT"):
...
for rec in recs:
    ...
    for b in rec.get('suspected_bugs') or []:
        cur.execute("INSERT INTO bug(func_addr,func_name,desc,location,severity,confidence,why,status,bug_class,leak_back,disclosure_source,reachability,guarded_by,impact) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
          (addr,nm,b.get('desc'),b.get('location',''),b.get('severity',''),b.get('confidence','low'),b.get('why',''),'open',b.get('bug_class',b.get('pattern','oob')),
           b.get('leak_back'),b.get('disclosure_source'),b.get('reachability'),b.get('guarded_by'),b.get('impact'))); n_b+=1
con.commit()
