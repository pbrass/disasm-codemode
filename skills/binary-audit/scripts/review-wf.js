...
4. DESK-CHECK the body for ALL of: OOB read/write; integer overflow/truncation/sign; off-by-one; UAF/double-free/refcount; TOCTOU/race (incl. a 2nd vCPU mutating shared state mid-operation); uninitialized-disclosure (the lens above); and these classes that prior passes UNDER-COVERED — hunt them explicitly:
   - **null-deref** (we demonstrated this class as live PSODs and the audit MISSED them): a callee return / `*_Alloc` / lookup / `*_Get` that can return NULL or an error, then dereferenced WITHOUT a check. If the attacker can influence the dereferenced address (NULL-page mappable, or a partly-controlled offset) it is corruption/control, not just a crash — say which.
   - **div-zero (#DE)**: an attacker-influenced value used as a divisor or modulus with no proven-nonzero guard (e.g. a guest MSS/segment-size/count field → `div`/`idiv`/`%`).
   - **uninit-use**: an uninitialized stack/heap value USED as a size/index/pointer/length (corruption) — distinct from uninit-DISCLOSURE (leaked). Check every local used before a definite assignment on some path.
   - **type-confusion**: an attacker- or checkpoint-controlled tag/handle/type/opcode field that selects which struct interpretation, union arm, or handler runs, without validating it matches the actual object.
   - **logic (privileged-userworld targets only)**: if this function runs in a privileged host process (VMX/hostd/guestOps) and touches a filesystem path, spawns a helper/command, or makes a privilege/credential/auth decision from attacker-influenced input — check command/argument injection, path traversal, symlink/TOCTOU file races, and missing privilege checks. These are escape-class even with zero memory corruption.
   For ANY read / over-read / uninitialized field you flag, you MUST classify two things or it is not actionable: (a) LEAK-BACK — does the data reach the attacker (reaches-attacker) or is it consumed internally and discarded (e.g. it only drives a checksum/validation/length decision → NOT a leak)? An OOB read whose result is discarded is at most a fault/DoS, not an info-leak. (b) REACHABILITY — which actor BOTH supplies the input AND reads/triggers the output: a guest VM, the userworld VMX process, a rogue remote/storage peer, or already-privileged host-local root. NOTE: functions named `*Cpt*`/`*Checkpoint*`/`*Restore*`/`*Load*`/`*SaveState*` are the **checkpoint/migration** path = forged-checkpoint by the trusted VMX or vMotion source = **host-local/migration reachability, NOT guest** — tag them so and do not inflate them to guest→host.
...
- ALWAYS set 'impact' to the CONCRETE attacker-OBSERVABLE outcome (host-psod / host-rce / host-mem-corruption / guest-readable-leak / vmx-rce / vmx-crash / privesc / dos-other / none-or-guarded). State what the attacker ACTUALLY GETS, not the mechanism. If the static defect is real but a runtime guard defeats it, or the over-read is discarded, or it can't be triggered by the actor in scope → impact = none-or-guarded with low confidence. This is the single biggest calibration lever: prior passes over-produced 'confirmed-violable' findings that were static-loose but harmless — forcing the observable impact kills those at review time.
