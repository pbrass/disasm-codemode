...
_excl = _prof.get('seed_exclude_regex')          # optional: drop matched names from the seed set (e.g. checkpoint/migration)
SEED_EXCL_RE = re.compile(_excl) if _excl else None
...
def main():
    ...
    seeds = [a for a in F if SEED_RE.search(F[a]['name']) and not (SEED_EXCL_RE and SEED_EXCL_RE.search(F[a]['name']))]
    print(f"seeds={len(seeds)} (of {len(F)})" + (f" [excluded by seed_exclude_regex]" if SEED_EXCL_RE else ""), file=sys.stderr)
    # BFS distance seed->f
