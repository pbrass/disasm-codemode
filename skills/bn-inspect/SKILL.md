...
| script | task | example |
|--------|------|---------|
| `bn-decompile` | decompile one function to HLIL (+ optional `--grep REGEX`/`--asm`) | `bn-decompile --file /tmp/target ParseRequest --grep memcpy --context 2` |
| `bn-find`  | find functions by name substring/`--regex`, or `--addr` → what's there | `bn-find --file /tmp/target --regex '^Handle.*Request'` |
| `bn-xrefs`     | callers + callees + call-sites of a function/`--addr` | `bn-xrefs --file /tmp/libsrp.so --addr 0x405e50` |
| `bn-strxref`   | find a string → the functions that reference it (`--decompile` to dump them) | `bn-strxref --file /tmp/target 'invalid request' --decompile` |
| `bn-scansec`   | list sections; `--read ADDR --len N` hexdump; `--section S --strings`/`--ptrs` | `bn-scansec --file /tmp/target --section .data --ptrs` |
