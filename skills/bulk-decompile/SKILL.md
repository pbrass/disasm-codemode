...
bn-bulk-decompile --bv-match target --out ./target_hlil --all --asm

# only the pre-auth reachable closure from chosen entry points (HLIL only):
bn-bulk-decompile --bv-match target --out ./target_hlil \
        --entry ParseRequest,HandleAuth,ParseRequestControls
...
bn-bulk-decompile --bv-match target --product appliance --build 1234567 \
        --root ./decomp --all --asm
```
