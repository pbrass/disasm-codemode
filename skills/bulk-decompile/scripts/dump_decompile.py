...
e.g.  decomp/appliance/1234567/target/ParseRequest.hlil.c
...
Usage:
  dump_decompile.py --product appliance --build 1234567 --bv-match target --root ./decomp --all [--asm]
  dump_decompile.py --product appliance --build 1234567 --bv-match target --root ./decomp \
                    --entry ParseRequest,HandleAuth [--asm] [--binary target]
...
def main():
    ...
    ap.add_argument("--bv-match", required=True, help="substring of the open GUI tab name to bind (e.g. target)")
    ...
    ap.add_argument("--product", help="product, e.g. appliance / firmware (with --build composes the structured path)")
