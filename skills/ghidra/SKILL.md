...
| `gh-find` | find functions by name substring/`--regex`, or `--addr` → what's there (`--no-imports`) | `gh-find --file /tmp/vmdird --regex '^Srv_.*[Ss]rp'` |
| `gh-xrefs` | callers + callees + call-sites of a function/`--addr` | `gh-xrefs --file /tmp/libsrp.so --addr 0x101379 --callers` |
| `gh-strxref` | find a string → the functions that reference it (`--decompile` to dump them) | `gh-strxref --file /tmp/vmdird 'SASL start failed' --decompile` |
