export const meta = {
  ...
  description: 'Contract-inference memory-safety review of 25 Vmxnet3/E1000/ENS guest-datapath vmkernel functions -> precondition ledger records',
...
2. CONTRACT INFERENCE: for EVERY memory access with a computed index/offset, and EVERY sink call (memcpy/memmove/memset/str*/*_Alloc/Pkt_* with a non-constant size), ask: "what must be TRUE for this to be memory-safe?" Each answer is a PRECONDITION. Systematically cover: index/length in-bounds; integer overflow/truncation in size or offset arithmetic; signedness (a signed value feeding a size/index); non-NULL; object-still-alive (UAF); lock-held (race/TOCTOU); and FIELD-CONSISTENCY (one guest field implying a bound on another — e.g. a length/offset/count/IHL/hdr-len header field used to index without being reconciled against the actual buffer size). The e1000 TSO bug in this same codebase was exactly a field-consistency precondition (guest IHL not reconciled with the descriptor header length) that was caller-owed and unchecked.
