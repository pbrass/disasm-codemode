...
description: >-
  ...
  binaries -> their dynamically AND statically/vendored-linked libraries + OS packages, POLYGLOT
  across ecosystems (native ELF/.so, Java jars incl. jars bundled inside WAR/EAR webapps, Python
  dist-info, Go buildinfo modules), resolves the
...
## Going wider: polyglot ecosystems (`scan_lang_deps.py`)
The three sources above are all **native** (ELF/.so). Modern appliances are POLYGLOT, and the single
richest untracked n-day surface is usually **not native** -- it's the Java jar layer. `scan_lang_deps.py`
adds three language ecosystems as first-class `library` rows (tagged `ecosystem`), reusing the same
`cve`/`analysis`/`link` machinery:

- **java** -- every `*.jar` on disk **AND every jar bundled inside `*.war`/`*.ear` webapps**. The
  bundled-in-WAR jars are the payoff: no RPM/package manager records them, so an RPM-only SBOM misses
  them entirely (on one appliance this was ~1,650 jar artifacts, ~950 third-party, **0** of which the
  C/RPM SBOM had). Version from the jar filename (fallback MANIFEST `Implementation-Version`). This is
  where the marquee Java CVEs live (log4j/spring/jackson/commons-*/bouncycastle/nimbus/xmlsec/woodstox).
- **python** -- `dist-info`/`egg-info`/`PKG-INFO` Name+Version across every interpreter's site-packages
  (appliance-management/firstboot tooling).
- **go** -- Go is **statically linked into ELF binaries**, so there are no loose Go files: it enumerates
  Go ELF binaries (the `Go buildinf:` marker) and extracts their embedded **buildinfo modules**
  (`go version -m`, or an offline `strings`-parse of the `dep\t<mod>\t<ver>` records since the target
  usually has no go toolchain). Each module is `link`ed to the carrying binary.

Vendor-internal / `*-SNAPSHOT` artifacts are tagged `vendor-internal` in `notes` (filter them out to
get the third-party CVE surface). Query a domain with `WHERE ecosystem='java'`; the money views span
all ecosystems. Needs a live `SBOM_HOST`. It runs last in `rebuild.sh` (stage 6).
