...
CREATE TABLE IF NOT EXISTS library (
    soname          TEXT PRIMARY KEY,   -- native: soname (liblber-2.4.so.2); polyglot: ecosystem-qualified,
                                        -- VERSION-qualified key so each distinct version is its own row (a jar
                                        -- present in N versions across services -> N rows; required for CVE mapping
                                        -- where one version is vulnerable and another is fixed):
                                        --   java:<artifact>@<ver>  py:<pkg>@<ver>  go:<module-path>@<ver>  npm:<pkg>@<ver>
                                        -- (`version` column duplicates the @<ver> for easy filtering; group by the
                                        --  pre-@ substring to roll up all versions of one component.)
    path            TEXT,
    version         TEXT,               -- resolved real version. native: build-path/strings (NOT the RPM);
                                        --   java: jar filename/MANIFEST; python: dist-info; go: buildinfo (go version -m)
    ecosystem       TEXT DEFAULT 'native',  -- native | java | python | go | npm  (language/packaging domain)
    upstream        TEXT,
    github_url      TEXT,
    provenance      TEXT,               -- build path / source tree / on-disk path / containing WAR or Go binary
    changed_in_patch INTEGER,
    -- WORK TRACKING: todo -> version-resolved -> cves-mapped -> audited
    audit_status    TEXT DEFAULT 'todo',
    notes           TEXT
);
-- `ecosystem` makes the library table POLYGLOT: Java jars, Python dist-info packages, Go buildinfo
-- modules and native .so all coexist as first-class components reusing the SAME cve/analysis/link/
-- upstream_fix machinery. Native ELF deps come from scan_static_deps.py (banner-grep) + the ldd
-- graph; language deps from scan_lang_deps.py. Go is special: it is STATICALLY linked into ELF
-- binaries, so its modules are extracted from each binary's embedded buildinfo (`go version -m`,
-- or parsed offline since the target usually lacks the go toolchain) and linked to that binary.
-- Query one domain with `WHERE ecosystem='java'`; the money views span all ecosystems.
