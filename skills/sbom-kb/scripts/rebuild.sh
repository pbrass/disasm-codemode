...
if [ -n "$SBOM_HOST" ]; then
  ...
  echo "[5/6] scan_dlopen_callers.py (which files import dlopen -> resolve targets with gh-/bn-callsites)..."
  python3 scripts/scan_dlopen_callers.py || true
  echo "[6/6] scan_lang_deps.py (POLYGLOT: Java jars incl. bundled-in-WAR, Python dist-info, Go buildinfo)..."
  python3 scripts/scan_lang_deps.py || echo "  (skipped -- see warning above)"
else
  echo "[2-6/6] SKIPPED -- set SBOM_HOST=<ssh-alias of a live target host> for versions + static + dlopen + polyglot passes"
fi
echo "--- v_completeness ---"
