...
def test_sbom_kb():
    ...
    try:
        ...
        # 7. sbom-kb must stay target-neutral: no vendor/product words at all.
        #    (Engagement-identifying tokens are enforced repo-wide by test_repo_hygiene.)
        for dirpath, _, filenames in os.walk(SBOMKB):
            ...
            for fn in filenames:
                if fn.endswith((".py", ".sql", ".md", ".sh")):
                    ...
                    for leak in VENDOR_WORDS:
                        if (os.path.relpath(path, ROOT), leak) in HYGIENE_EXEMPT:
                            continue
                        uassert(f"sbom: no '{leak}' in {fn}",
                                leak not in content.lower(),
                                f"found '{leak}' in {path}")
        con.close()
    except Exception as e:
...
#
#  ENGAGEMENT_TOKENS -- name a specific client target, host, build or workspace.
#      These have NO legitimate use anywhere in the tool, so they are banned
#      repo-wide with no exemptions beyond the one functional case below.
#      Rationale: example command lines that name a real target binary leak the
#      direction of live, undisclosed research (naming the target daemon plus the
#      auth-protocol symbols we were grepping told a
#      reader exactly which component/protocol was under audit).
#
#  VENDOR_WORDS -- generic vendor/product names. These DO have legitimate use:
#      binary-audit and symbolicate ship named VMware/ESXi profiles, and
#      METHODOLOGY.md states the provenance of the ranking function. So they are
#      enforced only in sbom-kb, which is meant to be strictly target-neutral.
# The engagement deny-list is stored as truncated SHA-256 digests, not plaintext:
# a literal list of client/target/build names in a public repo is a stronger tell
# than the examples it was written to catch. Matching is therefore word-granular
# (see _hygiene_words) rather than substring -- so a token embedded in a larger
# identifier (e.g. "<target>_hlil") is NOT caught. Accepted: the realistic
# regression is a path or a bare mention, both of which are word-granular.
# To add a token: python3 -c "import hashlib,sys;print(hashlib.sha256(sys.argv[1]
# .encode()).hexdigest()[:20])" <token>
ENGAGEMENT_TOKEN_HASHES = {
    "f88bb2a11d9025be4df7", "89596c470a915fdd02f4", "eb260bb686f28ac82f0e",
    "5ab5ea352fee88e9894a", "eedc9e9b7d4d7b1a0f57", "9932c9817dffd54f671d",
    "152b945018bcc8e6a2a7", "596b3cdfdae6e7e5b6cb", "53f1f104a2f078ad1fa2",
    "92b2bdb2d7ef833cd926", "99844cc00ff1c9a28f2a", "d2bfbdfef948cc8f2fbe",
    "ed8f5ebb84b3e7e1627a", "3b18b77093a509c9c562",
}
VENDOR_WORDS = ["vmware", "esxi", "vsphere", "photon", "likewise", "vmx"]


def _hygiene_words(content):
    """Word-granular candidate tokens from file content, lowercased."""
    return set(re.findall(r"[a-z0-9_.]{4,}", content.lower()))

# (relpath, vendor-word) -> reviewed exemptions for the sbom-kb vendor-word check.
HYGIENE_EXEMPT = {
    # Functional vendor-internal-jar classifier: the token list IS the feature.
    ("skills/sbom-kb/scripts/scan_lang_deps.py", "vmware"),
    ("skills/sbom-kb/scripts/scan_lang_deps.py", "vsphere"),
}
# relpaths exempt from the repo-wide engagement-token check, with the reason.
HYGIENE_EXEMPT_FILES = {
    # Same classifier: its regex names vendor product lines to tag internal jars.
    "skills/sbom-kb/scripts/scan_lang_deps.py",
}


def test_repo_hygiene():
    """No engagement-identifying tokens anywhere in the repo (it is public)."""
    exts = (".py", ".sql", ".md", ".sh", ".js", ".json", ".txt")
    scanned = 0
    for dirpath, dirnames, filenames in os.walk(ROOT):
        ...
        for fn in filenames:
            ...
            if rel == os.path.join("tests", "run_tests.py") or not fn.endswith(exts):
                continue   # this file necessarily contains the token list itself
            try:
                content = open(path, errors="replace").read().lower()
            except OSError:
                continue
            scanned += 1
            if rel not in HYGIENE_EXEMPT_FILES:
                hits = sorted(w for w in _hygiene_words(content)
                              if hashlib.sha256(w.encode()).hexdigest()[:20]
                              in ENGAGEMENT_TOKEN_HASHES)
                uassert(f"hygiene: no engagement tokens in {rel}", not hits,
                        f"{rel} contains engagement token(s): {hits}")
            # absolute home paths leak the operator's workspace layout
            uassert(f"hygiene: no /home/<user> path in {rel}",
                    "/home/" not in content,
                    f"{rel} contains an absolute /home/ path")
    uassert("hygiene: scanned a plausible number of files", scanned > 100,
            f"only scanned {scanned} files -- walk is probably broken")
